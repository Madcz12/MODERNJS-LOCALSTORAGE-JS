...
const inDanhSachSV = arr => {
    ...
    const trstring = arr.map(item =>
        `<tr>
            ...
            <td>${item.email}</td>
            <td>${item.gioitinh}</td>
            <td>${item.ngoaingu ? "<span class='text-success'>Tener</span>" : "<span class='text-danger'>No son</span>"}</td>
            <td>${item.tinhoc ? "<span class='text-success'>Tener</span>" : "<span class='text-danger'>No son</span>"}</td>
            <td>${item.qpan ? "<span class='text-success'>Tener</span>" : "<span class='text-danger'>No son</span>"}</td>
            <td><button type='button'  class="btn btn-primary" onclick='suaSV("${item.ma}")'>Arreglar</button>
            <button type='button' class="btn btn-danger"  onclick='xoaSV("${item.ma}")'>Eliminar</button></td>
        </tr>`).join('');
...
const xoaForm = () => {
    ...
    isUpdate = false;
    saveButton.textContent = "Agregar nuevo";
    masv.disabled = false;
}
